...
class Transformer(nn.Module):
    ...
    def compute_and_register_svd(self, rank=64):
        ...
        Registers the projection matrix to all residual blocks.
        """
        print(f"Computing SVD with rank {rank}...")

        # Collect weights from all ResidualAttentionBlocks
        weights = []
        ...
        # Perform SVD
        # torch.linalg.svd returns U, S, Vh. Vh shape is [width, width], rows are eigenvectors.
        ...
        # Create Projection Matrix: B.T
        # Shape: [width, rank]. Use this to project input x: (x @ proj_matrix)
        proj_matrix = basis.T.to(dtype=W_all.dtype)  # Convert back to original dtype

        # Register to all blocks as buffer
        for block in self.resblocks:
            block.register_buffer('proj_matrix', proj_matrix)

        print("SVD computation done. Projection matrix registered to all blocks.")
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        out, x, orth_loss = self.transformer(x)
        ...
        # This only returns CLIP features and orth_loss
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        return self.visual(image.type(self.dtype))  # Returns (features, cls_tokens, orth_loss)
